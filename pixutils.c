...
void pixMap_write_bmp16(pixMap *p,char *filename){
 BMP16_map *bmp16=BMP16_map_init(p->height,p->width,0,5,6,5);
 ...
 for(int i=0;i<p->height;i++){
		for(int j=0;j<p->width;j++){
			const int pix_i=p->height-i-1;
			const int pix_j=j;
			uint16_t bmp16Pixel=0;
   bmp16Pixel  =  p->pixArray[pix_i][pix_j].b >> (8-Bbits);
   bmp16Pixel |= (p->pixArray[pix_i][pix_j].g >> (8-Gbits)) << Bbits;
	  bmp16Pixel |= (p->pixArray[pix_i][pix_j].r >> (8-Rbits)) << (Bbits+Gbits);
	  bmp16Pixel |= (p->pixArray[pix_i][pix_j].a >> (8-Abits)) << (Bbits+Gbits+Rbits);
	  bmp16->pixArray[i][j]=bmp16Pixel;
		}			
	}	
 BMP16_write(bmp16,filename);
 free(bmp16);
}	 
